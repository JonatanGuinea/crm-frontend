...
export default function ProjectDetailPage() {
  ...
  if (!project) return <div className="p-8 text-sm text-gray-500">Proyecto no encontrado</div>
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-2 mb-6 text-sm text-gray-400">
        <button onClick={() => navigate('/projects')} className="hover:text-gray-700">← Proyectos</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <dl className="space-y-3 text-sm">
              {project.budget != null && (
                <div>
                  <dt className="text-xs text-gray-400 uppercase mb-0.5">Presupuesto</dt>
                  <dd className="text-gray-800 font-medium">${Number(project.budget).toLocaleString('es-AR')}</dd>
                </div>
              )}
              {project.startDate && (
                <div>
                  <dt className="text-xs text-gray-400 uppercase mb-0.5">Fecha inicio</dt>
                  <dd className="text-gray-800">{new Date(project.startDate).toLocaleDateString('es-AR')}</dd>
                </div>
              )}
              {project.endDate && (
                <div>
                  <dt className="text-xs text-gray-400 uppercase mb-0.5">Fecha fin</dt>
                  <dd className="text-gray-800">{new Date(project.endDate).toLocaleDateString('es-AR')}</dd>
                </div>
              )}
              {project.description && (
                <div>
                  <dt className="text-xs text-gray-400 uppercase mb-0.5">Descripción</dt>
                  <dd className="text-gray-800 whitespace-pre-line">{project.description}</dd>
                </div>
              )}
            </dl>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            {quotes.length === 0 ? (
              <p className="px-5 py-4 text-sm text-gray-400">Sin presupuestos asociados</p>
            ) : (
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            {invoices.length === 0 ? (
              <p className="px-5 py-4 text-sm text-gray-400">Sin facturas asociadas</p>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
