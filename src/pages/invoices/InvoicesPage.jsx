            ...
            <table className="w-full text-sm">
              <thead className="bg-raised border-b border-line">
                <tr>
                  ...
                  <th className="hidden md:table-cell text-left px-4 py-3 text-xs font-medium text-fg-soft uppercase tracking-wide">Vencimiento</th>
              ...
              <tbody className="divide-y divide-line">
                {data?.data?.map(inv => (
                  <tr key={inv.id} className="hover:bg-raised">
                    <td className="hidden sm:table-cell px-4 py-3 text-fg-muted">#{inv.number}</td>
                    <td className="px-4 py-3 font-medium text-fg">
                      <Link to={`/invoices/${inv.id}`} className="hover:text-brand">{inv.title}</Link>
                    </td>
                    <td className="hidden sm:table-cell px-4 py-3 text-fg-soft">{inv.client?.name}</td>
                    ...
                    <td className="hidden md:table-cell px-4 py-3 text-fg-soft">${Number(inv.total).toLocaleString('es-AR')}</td>
