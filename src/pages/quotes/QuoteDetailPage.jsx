...
export default function QuoteDetailPage() {
  ...
  if (!quote) return <div className="p-8 text-sm text-gray-500 dark:text-gray-400">Presupuesto no encontrado</div>
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate('/quotes')} className="text-sm text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
          ← Presupuestos
        </button>
      </div>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 dark:text-gray-500 mt-0.5 text-sm">#{quote.number}</p>
        </div>
            ...
            <dl className="space-y-3 text-sm">
              <div>
                ...
                <dd>
                  ...
                  {quote.client?.company && <span className="text-gray-400 dark:text-gray-500 ml-1">· {quote.client.company}</span>}
                </dd>
              </div>
              {quote.project && (
                <div>
              ...
              <div>
                ...
                <dd className="text-gray-800 dark:text-gray-100">{quote.currency}</dd>
              </div>
              {quote.validUntil && (
                <div>
                  <dt className="text-xs text-gray-400 dark:text-gray-500 uppercase mb-0.5">Válido hasta</dt>
                  <dd className="text-gray-800 dark:text-gray-100">{new Date(quote.validUntil).toLocaleDateString('es-AR')}</dd>
                </div>
              )}
              <div>
                <dt className="text-xs text-gray-400 dark:text-gray-500 uppercase mb-0.5">Creado</dt>
                <dd className="text-gray-800 dark:text-gray-100">{new Date(quote.createdAt).toLocaleDateString('es-AR')}</dd>
              </div>
              {quote.notes && (
                <div>
                  <dt className="text-xs text-gray-400 dark:text-gray-500 uppercase mb-0.5">Notas</dt>
                  <dd className="text-gray-800 dark:text-gray-100 whitespace-pre-line">{quote.notes}</dd>
                </div>
              )}
            </dl>
          </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div className="border-t border-gray-100 dark:border-gray-700 px-5 py-4 space-y-1 text-sm text-right">
              ...
              {quote.taxRate > 0 && (
                <div className="text-gray-500 dark:text-gray-400">
                  IVA ({quote.taxRate}%): <span className="text-gray-900 dark:text-white font-medium">${taxAmount.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                </div>
              )}
